...
6. **teach 分类教学(恢复 2010 原版指令)**:`teach` / `teach word|sentence|syntax|logic|greeting|auto` / `teach word 关键词`(分类+关键词同行)。分类存 `memorise.category` 列,`auto` 由 `service.detect_category` 按内置问候词表自动判定(greeting/word 二选一,其余分类只能手指定)。**word/sentence/syntax/logic 影响匹配灵敏度**(`service.CATEGORY_THRESHOLDS`):word 0.4(最严谨,同旧版)/ sentence 0.3(整句沾边就答)/ logic 0.2(提到关键词即答)/ syntax 0.4 但先剔除虚词(`SYNTAX_STOPWORDS`)再算(教结构,语气词不干扰)。未分类(旧数据)一律 0.4,行为与旧版完全一致。`POST /Categories` 返回分类统计(旧数据 NULL 计 unclassified)。改这块先看 `service.py` 顶部常量
6b. **greeting 不是问答对,是单条问候语(开场白)**:teach greeting 单轮输入一句问候语(前端 `teachMarisa` greeting 分支),存储时 keyword/分词/raw_keyword 全清空,只留 answer,**不参与任何问答匹配**(`add()` 4.6 分支:空分词、跳过子集合并——空集是任何集合的子集,必须 `if tokens and ...` 才能合并)。`POST /Greeting` 随机返回一条 greeting 词条,前端 `onMounted` 访问网站时自动发(2010-2011 原版语义)。auto 判定为 greeting 的问答对只取 answer 存为问候语
