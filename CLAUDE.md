...
6. **teach 分类教学(恢复 2010 原版指令)**:`teach` / `teach word|sentence|syntax|logic|greeting|auto` / `teach word 关键词`(分类+关键词同行)。分类存 `memorise.category` 列,`auto` 由 `service.detect_category` 按内置问候词表自动判定(greeting/word 二选一,其余分类只能手指定)。**分类影响匹配灵敏度**(`service.CATEGORY_THRESHOLDS`):word 0.4(最严谨,同旧版)/ sentence 0.3(整句沾边就答)/ logic 0.2(提到关键词即答)/ syntax 0.4 但先剔除虚词(`SYNTAX_STOPWORDS`)再算(教结构,语气词不干扰)/ greeting 0.4 + **精确匹配多条时优先返回**(`reply()` 里 max 排序键)。未分类(旧数据)一律 0.4,行为与旧版完全一致。`POST /Categories` 返回分类统计(旧数据 NULL 计 unclassified)。改这块先看 `service.py` 顶部常量
