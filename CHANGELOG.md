...
- **资讯工具**:正则触发,优先级为精确匹配(用户教学)> 工具 > 分词匹配;内置离线工具——时间/日期(现在几点/今天日期/星期几)、计算器(二元运算,`safe_eval` 白名单求值,除零/超大数回落兜底话术);可插拔在线工具——天气(高德,`AMAP_KEY` 配置启用,请求方 IP 定位城市后查实时天气,无公网 IP 时静默降级)、百科(萌娘百科,`DICT_API` 配置启用,自动从问句提取词条名,取已审核的社区摘要前 120 字)、汇率(`EXCHANGE_API` 配置非空才注册,urllib+5s 超时,失败静默降级)
